...
describe("build", () => {
  ...
  it("emits dist with site assets and snapshot, no overrides leak", async () => {
    ...
    await access(path.join(DIST, "d", "1.4.1.js"));
    await access(path.join(DIST, "go.html"));
    const indexHtml = await readFile(path.join(DIST, "index.html"), "utf8");
    assert.match(indexHtml, /d\/1\.4\.1\.js/);
    assert.match(indexHtml, /styles\.css\?v=1\.4\.1/);
    ...
    assert.match(goHtml, /d\/1\.4\.1\.js/);
    ...
    assert.equal(JSON.parse(latest).version, "1.4.1");
